...
import { useCourseDetails, useCourseLessons, useMarkLessonComplete, useSaveWatchTime } from "@/lib/hooks/useCourses";
...
const studentNavItems = [
  ...
  { label: "My Courses", href: "/student/courses", icon: (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" /></svg>) },
  { label: "Course Player", href: "/student/player", icon: (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>) },
  { label: "Assignments", href: "/student/assignments", icon: (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>) },
  { label: "Certificates", href: "/student/certificates", icon: (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z" /></svg>) },
  { label: "Payments", href: "/student/payments", icon: (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z" /></svg>) },
];
...
function CoursePlayerContent() {
  ...
  const saveWatchTime = useSaveWatchTime();
  ...
  const handleTimeUpdate = useCallback((e: React.FormEvent<HTMLVideoElement>) => {
    ...
    if (now - lastSaveTimeRef.current > 5000) {
      ...
      if (activeLesson) {
        saveWatchTime.mutate({ lessonId: activeLesson, seconds: currentTime });
      }
    }
  }, [activeLesson, saveWatchTime]);

  const handleEnded = useCallback(() => {
    if (activeLesson) {
      handleMarkComplete();
    }
  }, [activeLesson, handleMarkComplete]);
      ...
      <DashboardLayout navItems={studentNavItems}>
        <div className="flex h-[calc(100vh-80px)]">
          <div className="flex-1">
            <VideoPlayerSkeleton />
            <div className="p-6">
              <Skeleton variant="text" height={28} width="40%" className="mb-3" />
    ...
    return (
      <DashboardLayout navItems={studentNavItems}>
        <div className="flex flex-col items-center justify-center h-screen">
          ...
          <p className="text-gray-500 mb-4">{courseErrorMsg?.message || error || "You may need to enroll in this course first."}</p>
  ...
  return (
    <DashboardLayout navItems={studentNavItems}>
      <div className="flex h-[calc(100vh-80px)]">
            ...
            <>
              <div className="relative bg-black group">
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/60 to-transparent p-6 pt-12">
                  <h1 className="text-white font-semibold text-xl md:text-2xl mb-2">{currentLesson.title}</h1>
                  <p className="text-white/80 text-sm md:text-base">{currentLesson.description}</p>
                  <div className="flex items-center gap-4 mt-3">
                    <span className="text-white/60 text-sm">{currentLesson.duration}</span>
                    <span className="text-white/40">•</span>
                    <span className="text-white/60 text-sm">Lesson {modules.flatMap((m) => m.lessons).findIndex((l) => l.id === activeLesson) + 1} of {totalLessons}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 bg-white p-6 overflow-y-auto">
                <div className="max-w-5xl mx-auto">
                  ...
                  <div className="border-t border-gray-200 pt-6">
                    <h3 className="font-semibold text-gray-900 mb-4 text-lg">Lesson Resources</h3>
                    <div className="space-y-3">
                      <a href="#" className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors group">
                        ...
                        <div>
                          <p className="font-medium text-gray-900">Lesson Slides</p>
                          <p className="text-sm text-gray-500">PDF presentation slides</p>
                        </div>
                      </a>
                      <a href="#" className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors group">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-500">Download source code</p>
                        </div>
                      </a>
                    </div>
                  </div>
                  ...
                  <div className="mt-6 flex flex-col sm:flex-row gap-3 pt-6 border-t border-gray-200">
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-2 px-4 py-3 text-gray-600 hover:text-gray-900 hover:bg-gray-50 rounded-xl transition-colors disabled:opacity-50 disabled:cursor-not-allowed border border-gray-200"
                    >
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {sidebarOpen && (
          <div className="fixed right-0 top-[80px] w-80 h-[calc(100vh-80px)] bg-white border-l border-gray-200 overflow-y-auto lg:relative lg:top-0 lg:w-80 lg:h-auto z-40">
            <div className="sticky top-0 bg-white p-4 border-b border-gray-200">
              ...
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-500">{completedLessons}/{totalLessons} completed</span>
                <span className="text-blue-600 font-semibold">{Math.round((completedLessons / totalLessons) * 100)}%</span>
              </div>
        ...
        {!sidebarOpen && (
          <button
            ...
            className="fixed right-4 bottom-4 p-3 bg-blue-600 text-white rounded-full shadow-lg hover:bg-blue-700 transition-colors lg:hidden z-50"
          >
